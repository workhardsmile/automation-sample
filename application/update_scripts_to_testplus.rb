...
class ImportAutoScript
  ...
  def initialize
    ...
    #@url ="http://marquee.dev.activenetwork.com/import_data/import_automation_script"
    @url ="http://192.168.28.210/import_data/import_automation_script"
    @auto_case_array = []
  end
  ...
  def update_testlink
    begin
      ...
      RestClient.post("http://192.168.28.210:8002/testlink/update_status", data.to_json,{"Content-Type" => "application/json"})
    rescue Exception => e
  ...
  def update_script_testlink(script_name,case_array)
    begin
      ...
      RestClient.post("http://192.168.28.210:8002/testlink/update_automation_test_name", data.to_json, {"Content-Type" => "application/json"})
    rescue Exception => e
        ...
        if File.exist? "#{folder}/config.yml"
          ...
          begin
            ...
            if config and is_script_status_valid?(formart_status(config["status"]))
              ...
              if as["case_id"].size > 0
                xml = create_xml(as["script_name"],as["plan_name"],as["status"],as['comment'],as["owner"],as["timeout_limit"],strProject,strDriver,as["tags"],as["case_id"])
                post @url,xml.to_s
