def action_1
  ...
  it "IH598_settings-share_info for unsign test" do
    $parameters["unsign_test"].each do |test_data|
      ...
      expect(response["data"].merge(test_data["expected"]["data"])==response["data"]).to be_truthy
    end
  end
  
  it "IH599_settings-share_info for signin test" do
