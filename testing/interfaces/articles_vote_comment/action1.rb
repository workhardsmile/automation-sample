def action_1
  ...
  it "IH574_articles-vote_comment for unsign test" do
  ...
  it "IH575_articles-vote_comment for signin test" do
    api_base.set_user_token($parameters["user"]["mobile"],$parameters["user"]["password"])
    $parameters["signin_test"].each do |test_data|
      $step_array = $step_array << test_data
      if test_data["request"]["comment_id"].to_i == 0
        test_data["request"]["comment_id"] = Helper::IronHideDbUtil.get_one_valid_comment_id_unless_mobile($parameters["user"]["mobile"]) 
      elsif test_data["request"]["comment_id"].to_i == -1
        test_data["request"]["comment_id"] = Helper::IronHideDbUtil.get_one_valid_comment_id_by_mobile($parameters["user"]["mobile"])
      end
      response = api_base.post_by_api($parameters["api_path"],test_data["request"])
      Common.logger_info response
      expect(response["code"]).to eq(test_data["expected"]["code"])
