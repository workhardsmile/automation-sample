def action_1
  ...
  it "IH588_notifications-show for unsign test" do
  ...
  it "IH589_notifications-show for signin test" do
    api_base.set_user_token($parameters["user"]["mobile"],$parameters["user"]["password"])
    $parameters["signin_test"].each do |test_data|
      $step_array = $step_array << test_data
      test_data["request"]["notification_id"] = Helper::IronHideDbUtil.get_one_notification_id_by_mobile($parameters["export_user"]["mobile"]) if "#{test_data["request"]["notification_id"]}" == "0"
      test_data["request"]["notification_id"] = Helper::IronHideDbUtil.get_one_notification_id_by_mobile($parameters["user"]["mobile"]) if "#{test_data["request"]["notification_id"]}" == "1"
      response = api_base.get_by_api($parameters["api_path"],test_data["request"])
      Common.logger_info response
      expect(response["code"]).to eq(test_data["expected"]["code"])
      expect(response["message"].strip).to eq(test_data["expected"]["message"].strip)
