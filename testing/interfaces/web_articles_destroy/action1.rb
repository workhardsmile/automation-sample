def action_1
  ...
  it "IH676_articles-destroy for unsign test" do
  ...
  it "IH677_articles-destroy for signin test" do
    api_base.set_user_token($parameters["user"]["mobile"],$parameters["user"]["password"])
    $parameters["signin_test"].each do |test_data|
      $step_array = $step_array << test_data
      if "#{test_data["request"]["post_id"]}" == "" 
        response = api_base.post_by_api(test_data["precondition"]["api_path"],test_data["precondition"]["request"])
        test_data["request"]["post_id"] = Helper::IronHideDbUtil.get_one_valid_article_id_by_mobile($parameters["user"]["mobile"])
      end
      response = api_base.delete_by_api($parameters["api_path"],test_data["request"])
      Common.logger_info response
      expect(response["code"]).to eq(test_data["expected"]["code"])
