def action_1
  ...
  it "IH920_features-destroy for unsign test" do
  ...
  it "IH921_features-destroy for signin test" do
    $parameters["signin_test"].each do |test_data|
      ...
      expect(response["code"]).to eq(test_data["expected"]["code"])
      expect(response["message"]).to eq(test_data["expected"]["message"])
      expect(response["data"]["end_time"] != nil).to be_truthy if "#{test_data["expected"]["code"]}"=="1"
    end
  end
end
