def action_1
  ...
  it "IH641_topics-follow for unsign test" do
  ...
  it "IH642_topics-follow for signin test" do
    ...
    $parameters["signin_test"].each do |test_data|
      ...
      Common.logger_info response
      expect(response["code"]).to eq(test_data["expected"]["code"])
      expect(response["message"].strip.include?(test_data["expected"]["message"].strip)).to be_truthy
    end
  end
end
