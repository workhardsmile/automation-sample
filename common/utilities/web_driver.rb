...
module  WebDriver
  class << self  
    ...
    def start_remote_browser(language=nil)
      # accept browser types "ie", "firefox", "chrome"
      if $platform == 'firefox'
        caps = Selenium::WebDriver::Remote::Capabilities.firefox
        $driver = Selenium::WebDriver.for(:remote, :desired_capabilities => :firefox,:url => "http://127.0.0.1:443/wd/hub/")
      ...
      $driver.manage.window.resize_to(max_width, max_height)
        ...
        if $platform == 'chrome'
          ...
          $driver = Selenium::WebDriver.for(:remote, :desired_capabilities => caps,:url => "http://127.0.0.1:443/wd/hub/")
          $driver.get "about:blank"
        else
          $driver = Selenium::WebDriver.for(:remote, :desired_capabilities => $platform.to_sym,:url => "http://127.0.0.1:443/wd/hub/")
          $driver.get "about:blank"
          max_width, max_height = $driver.execute_script("return [window.screen.availWidth, window.screen.availHeight];")
        $driver.manage.window.resize_to(max_width, max_height)
        end
    ...
    def start_browser(option={})
      ...
      when "iphone","iphone_plus","ipad","android","ios_reality","ios_simulator","android_reality","android_simulator"
        send "start_mobile_#{$OS.to_s}", option
      end
    end
    ...
    def start_mobile_windows(option={})
    ...
    def start_mobile_macosx(option={})
      device = option["device"]
      Common.logger_info "Execute - Start Appium to test app on mobile devices"
      ...
      sleep(6)
      $appium_driver = Appium::Driver.new(caps: capabilities['caps'], appium_lib: {server_url: capabilities['appium_lib']['server_url'], session_id: '1143a53b-8cfc-4c03-bc95-366d394facea', wait: (capabilities['caps']['newCommandTimeout'].to_i rescue 0)})
