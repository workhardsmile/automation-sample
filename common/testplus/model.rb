...
class ScriptTask
  ...
  def initialize(temp_schedule_script={})
    if temp_schedule_script!=nil && temp_schedule_script!={}
      time_now = Time.now
...
class AutomationScriptResult < ActiveRecord::Base
  ...
  def end?
    self.state.downcase == 'end' or self.state == 'done' or self.state == 'killed' or self.state == 'timeout' or self.state == 'not implemented' or self.state == 'network issue'
  end
